// unsigned b1 = 0b01100001;
// std::bitset<4> b2 = 0b0001;
// std::cout << b2 << '\n'; // 0001
// std::cout << (char)b1 << '\n'; // a

// Compression, Extending, StringStream are needs to be optimized
...
namespace woXrooX {
	...
	private:
		...
		void hash() {
			...
			for (int i = 0; i < this->data.length(); ++i) data_in_binary.append(std::bitset<8>(this->data[i]).to_string());

			//////////////// Appending One Bit To The Data Before Filling With 0's Reamianing Bits
			data_in_binary.append("1");

			//////////////// Filling With 0's
			for (int i = 0; i < zero_bits_size; ++i) data_in_binary.append("0");
			...
			for (int i = 0; i < data_512_bit_chunks_size; ++i) data_512_bit_chunks[i] = data_in_binary.substr(512*i, 512);
			...
			for (int i = 0; i < data_512_bit_chunks_size; ++i) {
				for(int j = 0; j < 16; ++j) data_32_bit_words[i][j] = data_512_bit_chunks[i].substr(32*j, 32);
			}
		...
		std::bitset<32> choice(std::bitset<32> e, std::bitset<32> f, std::bitset<32> g) { return (e & f) ^ ((~e) & g); }

		///// Majority
		// result = (a and b) xor (a and c) xor (b and c)
		std::bitset<32> majority(std::bitset<32> a, std::bitset<32> b, std::bitset<32> c) { return (a & b) ^ (a & c) ^ (b & c); }
